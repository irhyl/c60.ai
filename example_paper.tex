...
\begin{figure}[t]
  ...
  \caption{%
    \textbf{AutoML framework comparison} on four datasets
    (Iris, Wine, Breast Cancer, Digits), 2-fold $\times$ 2 seeds.
    Left: per-dataset accuracy.
    Right: mean accuracy ranking — C60.ai is highlighted in blue and achieves
    the best single-system accuracy on Digits (98.83\%, marked).%
  }
  \label{fig:automl}
  \vskip -0.1in
\end{figure}
...
OptunaSearch, BayesSearchCV, Successive Halving (SuccHalving),
Broad Random Search, Greedy Ensemble, AutoStack (stacked
generalisation), FeatEng AutoML, and Optuna Ensemble.
Protocol: stratified 2-fold CV $\times$ 2 seeds (4 folds per system--dataset pair).

\begin{table}[t]
  \caption{%
    AutoML comparison on 4 datasets. Mean accuracy (\%) and average rank.
...
C60.ai ranks 7th by mean accuracy on these four datasets, but achieves the
\textbf{best single-system accuracy on Digits} (98.83\%) — the
highest-dimensional and most structurally demanding task in this comparison.
No AutoML framework statistically significantly outperforms C60.ai on any
individual dataset (Wilcoxon $p > 0.05$ for all pairwise comparisons).

Ensemble-based systems (Optuna Ensemble, Greedy Ensemble, AutoStack) lead by
mean accuracy because they excel on low-dimensional tasks where ensembling
orthogonal base classifiers is nearly optimal.
These methods cannot discover the intra-pipeline structural motifs that benefit
high-dimensional data — motifs that C60.ai's graph search finds naturally.
